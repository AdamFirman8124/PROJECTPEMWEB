...
<div class="container" style="padding: 20px;">
    <div class="card card-custom" style="box-shadow: 0 4px 8px rgba(0,0,0,0.1); border-radius: 10px; overflow: hidden;">
        <div class="card-header" style="background-color: #007BFF; color: white; padding: 10px 20px;">
            <h3 style="margin: 0;">Formulir Pendaftaran Seminar</h3>
        </div>
        <div class="card-body" style="padding: 20px; background-color: #f8f9fa;">
            <form method="POST" action="{{ route('seminar.store') }}" enctype="multipart/form-data">
                ...
                <a href="{{ route('home') }}" class="btn btn-primary" style="width: 100%; margin-top: 10px;">Kembali ke Beranda</a>
            </form>
        </div>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        submitButton.addEventListener('click', function(event) {
            ...
            inputs.forEach(input => {
                if (!input.value.trim()) { // Memastikan input tidak hanya diisi whitespace
                    isFormValid = false;
                }
            });
            ...
            if (!isFormValid) {
                ...
                form.submit(); // Mengirim formulir jika semua validasi terpenuhi
            }
        });
